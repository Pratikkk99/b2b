...
export const LoginScreen: React.FC<AuthProps> = ({ onLogin }) => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6 relative overflow-hidden font-sans">
        ...
        <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl border border-slate-200 p-8 md:p-10 relative z-10 transform hover:scale-[1.01] transition-transform duration-500">
            <div className="text-center mb-8 flex flex-col items-center">
                 <Logo size="xl" className="mb-4" />
                 <h2 className="text-3xl font-black text-slate-900 mt-4">Welcome Back</h2>
            ...
            <div className="space-y-3 mb-8">
                <button 
                  ...
                  className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors font-semibold text-slate-700 group"
                >
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {(['member', 'coach', 'trainer', 'admin'] as UserRole[]).map(role => (
                            <button 
                                ...
                                className={`text-xs font-bold py-2.5 px-2 rounded-lg border transition-all uppercase ${selectedRole === role ? 'bg-brand-blue text-white border-brand-blue shadow-lg shadow-brand-blue/20' : 'bg-white text-slate-600 border-slate-200 hover:border-brand-blue/50'}`}
                            >
                                {role}
                            </button>
                        ))}
